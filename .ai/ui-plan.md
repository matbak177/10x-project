...
### Widok 4: Podgląd Fiszek

- **Nazwa widoku**: Review View
- **Ścieżka widoku**: `/flashcards`
- **Główny cel**: Umożliwienie użytkownikowi przegladanie fiszek
- **Kluczowe informacje do wyświetlenia**:
  - Lista fiszek.
- **Kluczowe komponenty widoku**:
...
1. **Generowanie (Generate)**: Zostaje przekierowany na `/generate`. Wkleja tekst i klika "Generuj".
1. **Przekazanie danych**: Po pomyślnej odpowiedzi z API, lista propozycji jest zapisywana w stanie klienta.
1. **Recenzja (Review)**: Użytkownik jest automatycznie przekierowywany na `/review`, gdzie przegląda i modyfikuje propozycje.
1. **Finalizacja**: Klika "Zapisz X fiszek". Aplikacja wysyła finalną listę do API.
1. **Powrót i potwierdzenie (Dashboard)**: Po sukcesie, użytkownik wraca na `/`, gdzie widzi powiadomienie "toast" o powodzeniu, a nowo dodane fiszki pojawiają się na liście dzięki automatycznemu odświeżeniu danych przez TanStack Query.
